...
Generation of set of impact configurations -- approach used here was to specify the problem enough that the IK has a single solution and to search for the solution using an iterative method. A better approach may be to use fuzzy logic \cite{something?}. Enforcement of cond 5 based upon zero slope. Equation \ref{eqn:zeroslopefriccone} -- range of step length and height?
...
Enforcement of condition 3-4? -- based upon enforcement of ground condition.

Coping with errors? -- No $\theta^c$ - set to end or start, depending on step up or down -- No feasible solution

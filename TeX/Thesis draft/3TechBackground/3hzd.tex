...
\begin{enumerate}[label=\bfseries Condition \arabic*, parsep=0pt]
	...
	\item $\Phi_\beta'\left(\theta_\beta^+\right) = R\Delta\left(\Phi_\alpha(\theta_\alpha^-)\right)\Phi_\alpha'\left(\theta_\alpha^-\right)$ -- The initial velocity under the constraint matches the the post-impact velocity following the previous constraint. \label{item:velinvariance}

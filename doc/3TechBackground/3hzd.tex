...
\subsection{Zero dynamics} \label{sec:zerodyn}
...
Under the assumption of perfect regulation of the virtual constraints, we can evaluate the zero dynamics by simple substitution into Equation~\ref{eqn:dynamics}:
\begin{equation*}
...
For typical walking motions, there exists a single point of maximum potential energy within each footstep corresponding to a zero crossing of $\gamma(\theta)$. If we denote this point the \textit{critical point}, $\theta^c$, for any given virtual constraint $\alpha$, there exists an initial velocity which satisfies
...
We build families of Bézier curves with the following formulation:

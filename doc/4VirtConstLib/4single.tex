...
It is not typically necessary to subject the decision variables to any additional bounds other than the $\Delta$KE and $\sigma(x)$ constraints. This is due to the well-behaved nature of the problem; large deviations always cost more in terms of input torque than small ones. However, it may be advantageous in terms of the computational time of the optimisation to subject the decision variables to ``reasonable'' bounds. In practice, it is often difficult to tell what is reasonable without testing the results of the optimisation. The decision variables may also be constrained to avoid poor behaviour of the cost function, e.g. where $\alpha(\theta)$ crosses zero many times over the interval $[\theta_0, \theta_f]$ (see Section \ref{sec:alphazerocrossing})
